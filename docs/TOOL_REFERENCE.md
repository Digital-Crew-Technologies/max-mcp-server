...
Historical catalog of the **original ~66 MCP tools** exposed by `max-mcp-server`. Each entry includes the underlying HTTP endpoint, required scope, key inputs, and a one-line description.

> ⚠️ **Stale — do not rely on this file alone.** The live tool surface is now ~154 tools; the newer domains (claire research, enrichment, intent signals, inbox autopilot, CRM/HubSpot, Notion, calendar/Cal.com, LinkedIn, email analytics, agent action drafts, webhook simulators) are **not** included below. The auto-generated table in the root `README.md` (regenerated by `pnpm docs:tools`) is the source of truth for the current surface; this file remains useful for the endpoint-and-scope details of the original domains.
