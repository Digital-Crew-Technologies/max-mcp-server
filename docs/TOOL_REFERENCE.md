...
| `linkedin_search_people` | `GET /api/v1/linkedin/search-people` | Search LinkedIn people via the user's connected account (keywords, filters, cursor pagination). Every call uses 1 of the account's daily search budget (default 50/day); 429 `SEARCH_QUOTA_EXCEEDED` when exhausted. |
