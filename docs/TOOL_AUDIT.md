...
> **⚠️ Historical snapshot.** This audit ran against the 63-tool era; the server now registers 167 tools. Regenerate against a live server with `node scripts/audit-endpoints.mjs`. See [`tools.json`](tools.json) for the current inventory.
