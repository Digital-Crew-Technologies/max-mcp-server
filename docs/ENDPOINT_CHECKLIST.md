...
(MCP server historically registered 64 tools — 60 wrapping unique API endpoints + 1 `wait_for_prospect_list` polling helper + 3 local admin tools `get_circuit_status`, `list_failed_requests`, `clear_failed_requests`. The live surface has since grown to ~154 tools across new domains — see the auto-generated table in the root `README.md`, regenerated by `pnpm docs:tools`.)
