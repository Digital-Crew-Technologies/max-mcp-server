...
This server exposes **~154 MCP tools** wrapping the Digital Crew Max Agent API (the live count is regenerated by `pnpm docs:tools`). This document explains how to verify each tool works, what's covered, and what isn't.

> Note: earlier revisions of this doc quoted 64 tools. The current domain coverage (campaigns, prospects, prospect-lists, organizations, accounts, unibox, AI agent, dashboard, Apollo, Explorium, admin, plus the newer claire/enrichment/intent/inbox-autopilot/CRM/notion/calendar/linkedin/email-analytics/agent-drafts/webhook-simulator domains) has grown well past that — treat the auto-generated table in the root `README.md` as the source of truth.
